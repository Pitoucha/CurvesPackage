...
###     3DNA v2.4.4-2019sep09, created and maintained by xiangjun@x3dna.org
...
rotation matrix. Coordinate transformation is performed by 'frame_mol' or
'rotate_mol'. Useful for 'local' structural alignment. The transformed
ensemble can be converted to an image using '#{$prg} blocview'
...
TXT
        ...
        def parse_options
            ...
            if opts[:frame]
                Utils.assert_file_exist(opts[:bpfile], "must specify a bpfile with the -b option") unless opts[:single]
    ...
    class BlocView
        def main
            ...
            puts "Total number of models: #{models.size}"
            ...
            run_blocview(opts, selfile)
            FileUtils.cp("blocview.r3d", opts[:r3dfile])

            if opts[:repr]      # just the representative model
                FileUtils.cp("blocview.png", opts[:imgfile])
            else                # begin_model..end_model within the ensemble
                if bstr_num < opts[:begin_model] || bstr_num > opts[:end_model]
                    bstr_num = opts[:begin_model] # using begin_model as 'best representative'
                    puts "Process model <##{bstr_num}>"
                    Utils.run_cmd("#{$x3dna}ex_str -#{bstr_num} -nmr #{ensemble} #{selfile} #{$msg}")
                    run_blocview(opts, selfile)
                    FileUtils.cp("blocview.r3d", opts[:r3dfile])
                end
                models.each do |m|
                    next if m == bstr_num # already counted
                    next if m < opts[:begin_model] || m > opts[:end_model]
        ...
        def run_blocview(opts, selfile)
            if opts[:only_nucleic_acids]
                Utils.run_cmd("get_part #{selfile} na-#{selfile}")
                FileUtils.cp("na-#{selfile}", selfile)
            end
            Utils.run_cmd("#{$x3dna}blocview #{opts[:bkview]} #{selfile} #{$msg}") # "blocview.r3d"
        end
...
Examples:
        ...
        #{$prg} #{sub} --repr -i 2kei-selected.png -e 2kei.pdb
...
TXT
        ...
        def parse_options
            ...
            opts = Trollop::options do
                banner txt
                opt :ensemble, "name of PDB ensemble file", :type => :string, :short => "-e"
                ...
                opt :begin_model, "first model to process", :default => 1
                opt :end_model, "last model to process", :default => 100
                opt :only_nucleic_acids, "only nucleic acids are includes", :default => false
