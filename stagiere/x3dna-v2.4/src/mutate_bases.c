...
/* more general, e.g., it includes CN3 in 3MA (3-methyl-adenine) */
static long is_frame_base_atom(char *atom_name)
{
    return !strchr(atom_name, '\'') && !strchr(atom_name, 'P');
}
                          ...
                          char **AtomName, double **xyz, char **Miscs, FILE * fp)
{
    ...
    long i, j, k, is_dna, snum, *sResSeq;
    ...
    for (i = ib; i <= ie; i++) {  /* for backbone atoms, as is */
        k = is_baseatom(AtomName[i]) || is_frame_base_atom(AtomName[i]);
        if (k)
            continue;
    ...
    for (i = 1; i <= snum; i++) {  /* for mutated base atom */
        k = is_baseatom(sAtomName[i]) || is_frame_base_atom(sAtomName[i]);
        if (!k)
            continue;
